...
export default function Sustainability() {
  return (
    <section className="relative bg-zneako-charcoal py-24 md:py-32 px-6 md:px-16 lg:px-24">
      <div className="max-w-2xl mx-auto text-center">
        ...
        <p className="mt-4 font-body text-sm text-zneako-sand/60 leading-relaxed">
      ...
      <div className="mt-16 md:mt-20 max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {SDGS.map((sdg) => (
          <div
            key={sdg.number}
            className="rounded-lg border border-zneako-green/25 bg-zneako-green/10 p-5"
          >
            <span className="font-display text-xs font-bold tracking-[0.1em] text-zneako-green">
              SDG {sdg.number}
            </span>
            <p className="mt-2 font-display text-sm font-semibold text-zneako-cream leading-snug">
              {sdg.title}
            </p>
            <p className="mt-2 font-body text-xs text-zneako-sand/55 leading-relaxed">
              {sdg.description}
            </p>
          </div>
        ))}
      ...
      <div className="mt-16 md:mt-20 max-w-5xl mx-auto border-t border-zneako-sand/10 pt-12 md:pt-16">
        <div className="flex items-center justify-between gap-4">
          <p className="font-body text-xs tracking-[0.15em] uppercase text-zneako-sand/50">
            Compliance Frameworks
          </p>
          <p className="font-body text-xs text-zneako-sand/35 italic">Working toward, ahead of launch</p>
        </div>
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {CERTIFICATIONS.map((cert) => (
            <div
              key={cert.name}
              className="rounded-lg border border-zneako-sand/10 bg-zneako-rubber/20 p-5"
            >
              <p className="font-display text-sm font-semibold text-zneako-cream">{cert.name}</p>
              <p className="mt-1.5 font-body text-xs text-zneako-sand/55 leading-relaxed">
                {cert.description}
              </p>
            </div>
          ))}
